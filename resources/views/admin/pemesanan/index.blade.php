                ...
                <div class="col-md-9">
                    <form method="GET" action="{{ route('admin.pemesanan.index') }}">
                        <div class="row g-2">
                            ...
                            <div class="col">
                                <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Cari nama / film">
                            </div>
                            <div class="col-auto">
                        ...
                        <tbody class="text-center">
                            @foreach ($pemesanans as $pemesanan)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-{{ $status === 'paid' ? 'success' : ($status === 'pending' ? 'warning' : 'secondary') }}">
                                            {{ ucfirst($status) }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('admin.pemesanan.updateStatus', [$pemesanan->id, 'paid']) }}" method="POST" class="d-inline form-confirm" data-nama="{{ $pemesanan->user->name }} – {{ $pemesanan->jadwal->film->judul }}" data-aksi="mengonfirmasi">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn btn-sm btn-success me-1">Konfirmasi</button>
                                        </form>
                                        <form action="{{ route('admin.pemesanan.updateStatus', [$pemesanan->id, 'pending']) }}" method="POST" class="d-inline form-confirm" data-nama="{{ $pemesanan->user->name }} – {{ $pemesanan->jadwal->film->judul }}" data-aksi="menolak">
                                            @csrf
                                            @method('PUT')
                        ...
                        <div class="modal-dialog {{ $bukti ? 'modal-lg' : '' }} modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-center">
                                    ...
                                    @if ($bukti)
                                        <img src="{{ asset('storage/' . $bukti) }}" class="img-fluid rounded shadow" style="max-height: 400px;">
                                    @else
